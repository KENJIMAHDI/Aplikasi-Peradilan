...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Perkara Perdata Khusus (PHI/Niaga)</h1>
        <p class="text-gray-500 mt-1">Daftar perkara perselisihan hubungan industrial dan kepailitan/HAKI.</p>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" 
     x-data="{ 
         showModal: false, 
         isEdit: false, 
         editData: { status: 'Diajukan', status_pembayaran: 'Belum Dibayar', jenis_perdata: 'PHI', nominal_panjar: 0 }, 
         tab: (new URLSearchParams(window.location.search).get('tab')) || '{{ session('active_tab', 'phi') }}', 
         searchQuery: '' 
     }">
    ...
    <div class="border-b border-gray-100 flex justify-between items-center pr-6 bg-gray-50/30 overflow-x-auto">
        <div class="flex">
            <button type="button"
                    @click="tab = 'phi'; const url = new URL(window.location); url.searchParams.set('tab', 'phi'); window.history.replaceState({}, '', url);" 
                    :class="{'border-emerald-500 text-emerald-600 font-semibold bg-white': tab === 'phi', 'border-transparent text-gray-500 hover:text-gray-700': tab !== 'phi'}" 
                    class="px-6 py-4 text-sm font-medium border-b-2 whitespace-nowrap transition-all duration-200 outline-none">
                Register PHI
            </button>
            <button type="button"
                    @click="tab = 'niaga'; const url = new URL(window.location); url.searchParams.set('tab', 'niaga'); window.history.replaceState({}, '', url);" 
                    :class="{'border-emerald-500 text-emerald-600 font-semibold bg-white': tab === 'niaga', 'border-transparent text-gray-500 hover:text-gray-700': tab !== 'niaga'}" 
                    class="px-6 py-4 text-sm font-medium border-b-2 whitespace-nowrap transition-all duration-200 outline-none">
                Register Niaga
            </button>
        </div>
        @can('manage-perkara')
        <button type="button" 
                @click="showModal = true; isEdit = false; editData = { status: 'Diajukan', status_pembayaran: 'Belum Dibayar', jenis_perdata: tab === 'niaga' ? 'Niaga' : 'PHI', nominal_panjar: 0, tanggal_daftar: '{{ date('Y-m-d') }}' }" 
    ...
    <div x-show="tab === 'phi'" class="p-0">
        <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <span class="text-xs text-gray-400 font-medium">Menampilkan data Register PHI</span>
        </div>
            <table class="w-full text-left text-sm text-gray-500">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse($perkara->where('jenis_perdata', 'PHI') as $item)
                    <tr class="hover:bg-emerald-50/20 transition-colors duration-150" x-show="searchQuery === '' || '{{ strtolower($item->nomor_register) }} {{ strtolower($item->penggugat) }} {{ strtolower($item->tergugat) }}'.includes(searchQuery.toLowerCase())">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            ...
                            <form action="/perdata-khusus/{{ $item->id }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center text-gray-500">Belum ada data register PHI.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <!-- Tab Niaga -->
    <div x-show="tab === 'niaga'" class="p-0" style="display: none;">
        <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <div class="relative w-64">
                <input type="text" x-model="searchQuery" class="block w-full pl-3 pr-3 py-2 border border-gray-200 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm bg-white" placeholder="Cari No. Perkara...">
            </div>
            <span class="text-xs text-gray-400 font-medium">Menampilkan data Register Niaga</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-gray-500">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse($perkara->where('jenis_perdata', 'Niaga') as $item)
                    <tr class="hover:bg-emerald-50/20 transition-colors duration-150" x-show="searchQuery === '' || '{{ strtolower($item->nomor_register) }} {{ strtolower($item->penggugat) }} {{ strtolower($item->tergugat) }}'.includes(searchQuery.toLowerCase())">
                        <td class="px-6 py-4 font-medium text-emerald-700">{{ $item->nomor_register }}</td>
                        <td class="px-6 py-4 text-gray-600">{{ $item->tanggal_daftar ? \Carbon\Carbon::parse($item->tanggal_daftar)->format('d/m/Y') : '-' }}</td>
                        <td class="px-6 py-4 text-gray-900 font-medium">{{ $item->penggugat }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center text-gray-500">Belum ada data register Niaga.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="showModal" 
        ...
        <div class="relative w-full max-w-xl bg-white rounded-xl shadow-2xl p-6 overflow-hidden my-auto border border-gray-100 max-h-[90vh] overflow-y-auto text-left" @click.away="showModal = false">
            <div class="flex justify-between items-center pb-3 border-b border-gray-100 mb-4">
                <h3 class="text-lg font-bold text-gray-800" x-text="isEdit ? 'Edit Data Register' : ('Tambah Register ' + (tab === 'niaga' ? 'Niaga' : 'PHI'))"></h3>
                <button type="button" @click="showModal = false" class="text-gray-400 hover:text-gray-600 text-2xl font-bold">&times;</button>
            </div>

            <!-- Body Form CRUD -->
            <div>
                <form :action="isEdit ? '/perdata-khusus/' + editData.id : '{{ route('perdata-khusus.store') }}'" method="POST">
                    @csrf
                    <template x-if="isEdit">
                        @method('PUT')
                    </template>
                    
                    <input type="hidden" name="jenis_perdata" :value="isEdit ? editData.jenis_perdata : (tab === 'niaga' ? 'Niaga' : 'PHI')">
                    <input type="hidden" name="kategori" :value="isEdit ? editData.jenis_perdata : (tab === 'niaga' ? 'Niaga' : 'PHI')" value="{{ request('tab', 'phi') }}">

                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700">Nomor Perkara</label>
                            <input type="text" name="nomor_perkara" :value="isEdit ? editData.nomor_register : ''" required placeholder="Contoh: 12/Pdt.Sus-PHI/2026/PN.Smg" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700">Pihak Penggugat / Pemohon</label>
                            <input type="text" name="penggugat" :value="isEdit ? editData.penggugat : ''" required placeholder="Nama Lengkap Penggugat" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700">Pihak Tergugat / Termohon</label>
                            <input type="text" name="tergugat" :value="isEdit ? editData.tergugat : ''" required placeholder="Nama Lengkap Tergugat" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm">
                        </div>
                        <div>
